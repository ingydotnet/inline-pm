...
Note that C<newSVrv()> doesn't create a reference, but returns a new SV
('obj') and makes 'obj_ref' a reference to it. Ultimately, 'obj_ref'
(which is the SV that C<new()> returns) holds a reference to the blessed
