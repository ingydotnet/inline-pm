...
use File::Spec;
use File::Spec::Unix;
use Fcntl qw(LOCK_EX LOCK_SH O_WRONLY O_CREAT);
use version;
use utf8;

use constant CAN_FLOCK => !(
    $^O eq 'VMS' ||
    $^O eq 'riscos' ||
    $^O eq 'VOS'
);
...
sub satisfy_makefile_dep {
    ...
    print $fh "*** AUTOGENERATED by Inline.pm ***\n\n";
    print $fh "This file satisfies the make dependency for ";
    print $fh "$o->{API}{module}\n";
    close $fh;
    return;
}
...
sub receive_code {
    ...
    if (ref $code eq 'CODE') {
        ...
        open my $fh, "<", $code
            or croak(
                $!{ENOENT}
                    ? M07_code_file_does_not_exist($code)
                    : M06_code_file_failed_open($code)
            );
        local $/;
        $o->{API}{code} = readline $fh;
    }
    else {
        $o->{API}{code} = $code;
    }
    ...
    if($o->{CONFIG}{REWRITE_CONFIG_FILE}) {
      if(-e File::Spec->catfile($DIRECTORY, $configuration_file)) {
       ...
       my $load_cfg = sub {
           ...
           flock($fh, LOCK_SH) if CAN_FLOCK;
           my $config = do { local $/; readline $fh };           
           close $fh;
...
sub create_config_file {
    ...
    for my $lib (@INC) {
        ...
        opendir my $dh, File::Spec->catdir($lib,"Inline")
    ...
    sysopen my $fh, $file, O_WRONLY | O_CREAT, 0666
        or croak M24_open_for_output_failed($file);
    flock($fh, LOCK_EX) if CAN_FLOCK;
    truncate($fh, 0);
    print $fh Inline::denter->new()
...
sub check_module {
    ...
    while ($auto_level <= 5) {
        ...
        my %inl;
        {
            open my $fh, "<", $inl or croak M31_inline_open_failed($inl);
            local $/;
            %inl = Inline::denter->new()->undent(readline $fh);
        }
...
END
    ...
    open my $fh, ">", File::Spec->catfile($o->{API}{build_dir},"REPORTBUG")
