...
for my $module (@modules) {
  ...
  my @test_result = run ( command => "$Config{make} test", verbose => 0);
  ok($test_result[0], "'make test' should succeed");

  # check that  '$Config{make} install' succeeds;
  my @install_result = run( command => "$Config{make} install", verbose => 0);
  ok($install_result[0], "'make install' should succeed");

  # check that  '$Config{make} realclean' succeeds;
  my @clean_result = run( command =>  "$Config{make} realclean", verbose => 0);
